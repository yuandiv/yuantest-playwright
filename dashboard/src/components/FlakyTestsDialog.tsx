...
interface FlakyTestsDialogProps {
  isOpen: boolean;
  onClose: () => void;
  lang: Lang;
  reports: RunReport[];
...
export function FlakyTestsDialog({
  isOpen,
  onClose,
  lang,
  reports,
  ...
  onRefresh,
  onClearFlakyHistory,
}: FlakyTestsDialogProps) {
  const [activeTab, setActiveTab] = React.useState<TabKey>('overview');
  const [causalGraph, setCausalGraph] = React.useState<CausalGraphData | null>(null);
  const [loadingCausalGraph, setLoadingCausalGraph] = React.useState(false);
  const [selectedImpact, setSelectedImpact] = React.useState<ImpactAnalysisData | null>(null);
  const [correlations, setCorrelations] = React.useState<any[] | null>(null);
  const [loadingCorrelations, setLoadingCorrelations] = React.useState(false);
  const [healthScore, setHealthScore] = React.useState<any | null>(null);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-[90vw] max-w-[1200px] max-h-[85vh] flex flex-col overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 flex-shrink-0">
          <h2 className="text-lg font-semibold text-gray-800">
            <i className="fas fa-bug mr-2 text-amber-500"></i>{t('flakyTestsTitle', lang)}
            <span className="text-xs text-gray-400 ml-2">{flakyTests.length} {t('items', lang)}</span>
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="border-b border-gray-100 px-6 flex-shrink-0">
        ...
        <div className="border-t border-gray-100 px-6 py-3 flex justify-end flex-shrink-0">
          <button
            onClick={onClose}
            className="px-4 py-2 bg-gray-100 text-gray-700 text-sm rounded-lg hover:bg-gray-200 transition-colors cursor-pointer"
          >
            {t('close', lang) || '关闭'}
          </button>
        </div>
