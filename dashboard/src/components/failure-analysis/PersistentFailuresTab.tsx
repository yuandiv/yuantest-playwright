...
interface PersistentFailuresTabProps {
  lang: Lang;
  reports: RunReport[];
}
...
export const PersistentFailuresTab: React.FC<PersistentFailuresTabProps> = ({ lang, reports }) => {
  ...
  const loadPersistentFailures = useCallback(async () => {
    setLoading(true);
    try {
      const data = await api.getFailureAnalysis('persistent');
      if (data && Array.isArray(data) && data.length > 0) {
        ...
        const fromReports = computePersistentFailuresFromReports(reports);
  ...
  const computePersistentFailuresFromReports = (reportList: RunReport[]): FailureItem[] => {
  ...
  if (failedItems.length === 0) {
    return (
      <div className="bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl p-6 text-center">
        ...
        <p className="text-gray-400 text-xs">{t('noPersistentFailures', lang) || '没有持续性失败的用例'}</p>
      </div>
    );
  }
    ...
    <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
      <div className="px-4 py-2 bg-red-50 border-b border-red-100">
        <h4 className="text-xs font-semibold text-red-700">
          ...
          {t('persistentFailures', lang) || '持续性失败'} ({failedItems.length})
        </h4>
        <p className="text-[10px] text-red-600 mt-0.5">
          {lang === 'zh' ? '连续失败3次及以上的用例' : 'Failed 3 or more times consecutively'}
        </p>
      </div>
                ...
                <div className="px-4 pb-2.5 border-t border-gray-100 pt-2">
                  {item.error && (
                    <div className="bg-red-50 rounded-lg p-2 mb-2">
                      <p className="text-[10px] font-medium text-red-600 mb-0.5">{t('errorMessage', lang) || '错误信息'}</p>
                      <p className="text-xs text-red-700 font-mono whitespace-pre-wrap">{item.error}</p>
                    </div>
                  )}
...
export default PersistentFailuresTab;
