...
interface FirstTimeFailuresTabProps {
  lang: Lang;
  reports: RunReport[];
}
...
export const FirstTimeFailuresTab: React.FC<FirstTimeFailuresTabProps> = ({ lang, reports }) => {
  ...
  const loadFirstTimeFailures = useCallback(async () => {
    setLoading(true);
    try {
      const data = await api.getFailureAnalysis('immediate');
      if (data && Array.isArray(data) && data.length > 0) {
        ...
        const fromReports = computeFirstTimeFailuresFromReports(reports);
  ...
  const computeFirstTimeFailuresFromReports = (reportList: RunReport[]): FailureItem[] => {
  ...
  if (failedItems.length === 0) {
    return (
      <div className="bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl p-6 text-center">
        ...
        <p className="text-gray-400 text-xs">{t('noFirstTimeFailures', lang) || '没有首次失败的用例'}</p>
      </div>
    );
  }
    ...
    <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
      <div className="px-4 py-2 bg-orange-50 border-b border-orange-100">
        <h4 className="text-xs font-semibold text-orange-700">
          ...
          {t('firstTimeFailures', lang) || '首次失败'} ({failedItems.length})
        </h4>
        <p className="text-[10px] text-orange-600 mt-0.5">
          {lang === 'zh' ? '仅失败过一次的用例' : 'Failed only once'}
        </p>
      </div>
              ...
              >
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="px-1.5 py-0.5 rounded text-[9px] font-medium bg-orange-100 text-orange-700">
                      {t('firstTime', lang) || '首次'}
                    </span>
                  </div>
                ...
                <div className="px-4 pb-2.5 border-t border-gray-100 pt-2">
                  {item.error && (
                    <div className="bg-orange-50 rounded-lg p-2 mb-2">
                      <p className="text-[10px] font-medium text-orange-600 mb-0.5">{t('errorMessage', lang) || '错误信息'}</p>
                      <p className="text-xs text-orange-700 font-mono whitespace-pre-wrap">{item.error}</p>
                    </div>
                  )}
                  {item.firstFailureTime && (
...
export default FirstTimeFailuresTab;
