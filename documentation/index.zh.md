...
- **Playwright 知识库** - 内置 7 大类 30+ 个错误模式，自动匹配注入 few-shot 示例
- **单次 LLM 调用** - 使用 `responseFormat: json_object` 强制 JSON 结构化输出
- **置信度校准** - 基于模式匹配、上下文完整度进行多维度置信度评分（0.6 × LLM置信度 + 加分项）
- **流式诊断** - SSE 实时推送推理过程，支持 `start`/`chunk`/`complete`/`error` 事件
- **批量聚类** - 基于 Jaccard 相似度 + 并查集算法自动识别同根因的批量失败
- **可操作修复建议** - 包含代码 Diff 和 Playwright 文档链接的结构化修复方案
- **缓存与持久化** - 内存缓存（LRU，100条，30分钟TTL）+ 磁盘持久化 `{dataDir}/diagnosis/`
