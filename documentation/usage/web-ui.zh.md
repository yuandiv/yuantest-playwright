...
Agent 代理面板提供 AI 驱动的测试创建和修复能力：

- **测试规划** - 输入功能描述生成结构化测试计划
  - 支持引用参考测试以保持代码风格一致
  - 支持引用 PRD 文档以对齐需求
  - 自动保存计划为 Markdown 文件
- **测试生成** - 将测试计划转换为 Playwright TypeScript 代码
  - 使用现代定位器（getByRole、getByText 等）
  - 生成独立可运行的测试场景
  - 支持自定义输出目录
- **测试修复** - 分析并修复失败测试
  - 多轮修复，逐步优化
  - 生成带置信度评分和统一 Diff 的补丁
  - 可选自动应用模式，即时修复
  - 安全：补丁仅应用于项目根目录内的文件
- **修复历史** - 查看历史修复操作
  - 追踪修复成功/失败率
...
- **项目上下文** - 自动检测的项目信息
  ...
  - 解析 Playwright 配置（baseURL、超时、视口）
  - 自动发现测试 Fixtures
