...
export class AgentOutputParser {
  ...
  // ─── 代码提取与文件保存（统一方法，消除 GeneratorService 与 ai-service 的重复） ──

  /** 从代码中提取测试名称并转为文件名友好的 slug */
  static extractTestNameFromCode(code: string): string | null {
    const describeMatch = code.match(/test\.describe\(['"](.+?)['"]/);
    if (describeMatch) {
      return this.generateSlug(describeMatch[1]) || null;
    }
    const testMatch = code.match(/test\(['"](.+?)['"]/);
    if (testMatch) {
      return this.generateSlug(testMatch[1]) || null;
    }
    return null;
  }

  /** 将文本转为文件名友好的 slug（最多 50 字符） */
  ...
  /**
   * 统一保存生成的代码到文件。
   * @param responseText  LLM 返回的文本（可含代码块）
   * @param outputDir     输出目录（不传则用 projectRoot + '/tests'）
   */
  static saveGeneratedCode(responseText: string, outputDir: string): string[] {
